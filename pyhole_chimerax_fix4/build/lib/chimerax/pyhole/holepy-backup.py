...
def parse_residue_tokens(s):
    ...
    for t in s.split(','):
        ...
        if not t:
            continue
        # e.g. "A2428/A" or "2428/A"  → (chain, resi)
        m = re.match(r'^[A-Za-z]{0,3}?(\d+)\s*/\s*([A-Za-z0-9])$', t)
        if m:
            toks.append((m.group(2), int(m.group(1))))
            continue
        # e.g. "A 2428" or "A:2428"
        m = re.match(r'^([A-Za-z0-9])\s*[:\s]\s*(\d+)$', t)
        if m:
            toks.append((m.group(1), int(m.group(2))))
            continue
        # e.g. "2428" (any chain)
...
def run_pyhole(
    ...
    probe=0.0, conductivity=1.5,  # accepted, not yet used in core math
    centerline="straight",
    ...
    """
    Compute pore profile and write outputs, returning file paths for the GUI.

    Returns: dict with keys 'csv', 'mesh_pdb', 'centerline_pdb'.
    """
    # 0) Get a PDB file path to read atoms from
    pdb_path = None
    if pdb:
        pdb_path = Path(pdb)
    elif session is not None and model is not None:
        # Export the selected model to a temp PDB
        from chimerax.core.commands import run as cxrun
        import tempfile

        # Normalize model id and choose a writable temp dir across versions
        id_str = str(getattr(model, 'id_string', getattr(model, 'id', 'model'))).lstrip('#')
        tmp_root = Path(getattr(session, 'user_data_dir', '') or
                        getattr(session, 'tmp_dir', '') or
                        tempfile.gettempdir())
        tmp_root.mkdir(parents=True, exist_ok=True)

        tmp = tmp_root / f"pyhole_tmp_{id_str}.pdb"
        model_spec = f"#{id_str}"
        cxrun(session, f'save "{tmp}" models {model_spec} format pdb')
        pdb_path = tmp

    else:
        raise ValueError("run_pyhole: provide 'pdb' path or (session and model).")

    # 1) Load atoms (respect hydrogen toggle)
    atoms = load_pdb_atoms(pdb_path, include_h=not noH)

    # 2) Parse selections and compute top/bottom centers
    top_sel = parse_residue_tokens(top)
    bot_sel = parse_residue_tokens(bottom)
    top_ca = ca_positions_for(atoms, top_sel)
    bot_ca = ca_positions_for(atoms, bot_sel)
    if top_ca.size == 0 or bot_ca.size == 0:
        raise ValueError("Could not find CA for top/bottom selections.")
    c_top = top_ca.mean(axis=0)
    c_bot = bot_ca.mean(axis=0)

    # 3) Optional custom VDW radii
    custom = {}
    if vdwjson:
        with open(vdwjson, 'r') as jf:
            custom = json.load(jf)
    ...
    # 4) Normalize occupancy choice from GUI
    ...
    if centerline == 'curved':
        rows, u, L = profile_curved(coords, radii, c_bot, c_top, step, eps, meta,
                                    curve_radius, curve_iters, hydroscale, electroscale, occ)
    else:
        rows, u, L = profile_straight(coords, radii, c_bot, c_top, step, eps, meta,
                                      adaptive, slope_thresh, max_refine, hydroscale, electroscale, occ)
    ...
    mesh_pdb(mesh_path, rows, axis_u=(c_top - c_bot), rings=rings)
    centerline_pdb(center_path, rows)

    return {"csv": str(csv_path), "mesh_pdb": str(mesh_path), "centerline_pdb": str(center_path)}
