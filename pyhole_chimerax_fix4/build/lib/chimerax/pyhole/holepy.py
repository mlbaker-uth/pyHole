...
class Atom:
    ...
    def __init__(self,x,y,z,name,resname,chain,resi,element,occ):
        ...
        self.resname=resname; self.chain=chain; self.resi=int(resi); self.element=element; self.occ=float(occ)
...
def profile_along_axis(atom_xyz, atom_r, c0, c1, step, eps, atom_meta,
    ...
    def eval_rows(vals):
        ...
        for s in vals:
            ...
            out.append({'s_A':float(s),'x':float(c[0]),'y':float(c[1]),'z':float(c[2]),
...
def profile_along_centers(atom_xyz, atom_r, centers, eps, atom_meta, hydro_scale, electro_scale, occupancy_metric):
    ...
    for i,c in enumerate(centers):
        ...
        rows.append({'s_A':float(s_acc),'x':float(c[0]),'y':float(c[1]),'z':float(c[2]),
                     'radius_A':float(rmin),'hydro_index':float(hyd),'electro_index':float(elec),
                     'contributors':tags})
...
def _trap_volume_A3(rows, probe_A):
    ...
    for i in range(len(rows)-1):
        ...
        r0=max(0.0, float(rows[i]['radius_A'])-probe_A); r1=max(0.0, float(rows[i+1]['radius_A'])-probe_A)
        vol+=0.5*(math.pi*r0*r0 + math.pi*r1*r1)*ds
    return vol

def _conductance_proxy_S(rows, conductivity_S_per_m):
    kappa=max(1e-12, float(conductivity_S_per_m)); rho=1.0/kappa
    R_pore=0.0; blocked=False
    for i in range(len(rows)-1):
    ...
    if blocked: return 0.0, float('inf'), R_access
    R_total=R_pore+R_access
    return (1.0/R_total), R_pore, R_access

# --- CLI entry (kept for parity/testing) ---
def main(args=None):
    ...
    if ca_top.size==0 or ca_bot.size==0: print("ERROR: Could not find CA atoms.",file=sys.stderr); return 3
    ...
    if ns.centerline=='straight':
        rows,u,L=profile_along_axis(coords,radii,c_bot,c_top,ns.step,ns.eps,metas,adaptive=ns.adaptive,
    ...
    # volumes & conductance
    volume_geom=_trap_volume_A3(rows,0.0); volume_access=_trap_volume_A3(rows,float(ns.probe))
    G_S, R_pore, R_access = _conductance_proxy_S(rows, ns.conductivity)
    G_nS = G_S * 1e9
    ...
    with open(summary_path,'w') as jf:
        json.dump({'pdb':str(Path(ns.pdb).resolve()),'top':ns.top,'bottom':ns.bottom,'centerline':ns.centerline,
                   ...
                   'occupancy_metric':ns.occupancy,'hydroscale':ns.hydroscale,'electroscale':ns.electroscale}, jf, indent=2)
    ...
    print(f"Wrote: {csv_path}"); print(f"Wrote: {pdb_center}"); print(f"Wrote: {pdb_mesh}"); print(f"Wrote: {summary_path}")
    return 0

# --- GUI entrypoint (used by your ChimeraX tool) ---
def run_pyhole(
    session=None, model=None, pdb=None, *,
    top="", bottom="",
    vdwjson="", outprefix="pyhole_out",
    step=1.0, eps=0.25, rings=24, noH=False,
    probe=0.0, conductivity=1.5,
    centerline="straight",
    adaptive=False, slope_thresh=0.5, max_refine=3,
    curve_radius=2.0, curve_iters=3,
    occupancy="hydro", hydroscale="raw", electroscale="raw",
    passable_json=""):
    """
    Compute pore profile and write outputs. Returns file paths (plus summary).
    """
    # 0) Resolve PDB path (GUI passes pdb=..., so we typically skip exporting)
    pdb_path = None
    if pdb:
        pdb_path = Path(pdb)
    elif session is not None and model is not None:
        # fallback export (UI should avoid calling this from worker thread)
        from chimerax.core.commands import run as cxrun
        import tempfile
        id_str = str(getattr(model, 'id_string', getattr(model, 'id', 'model'))).lstrip('#')
        tmp_root = Path(getattr(session, 'user_data_dir', '') or tempfile.gettempdir()); tmp_root.mkdir(parents=True, exist_ok=True)
        tmp = tmp_root / f"pyhole_tmp_{id_str}.pdb"
        cxrun(session, f'save "{tmp}" models #{id_str} format pdb')
        pdb_path = tmp
    else:
        raise ValueError("run_pyhole: provide 'pdb' or (session and model).")

    atoms=load_pdb_atoms(pdb_path, include_h=not noH)
    top_sel=parse_residue_tokens(top); bot_sel=parse_residue_tokens(bottom)
    top_ca=ca_positions_for(atoms,top_sel); bot_ca=ca_positions_for(atoms,bot_sel)
    if top_ca.size==0 or bot_ca.size==0: raise ValueError("Could not find CA for top/bottom selections.")
    c_top=top_ca.mean(axis=0); c_bot=bot_ca.mean(axis=0)

    custom={}
    if vdwjson:
        with open(vdwjson,'r') as jf: custom=json.load(jf)
    coords=np.array([[a.x,a.y,a.z] for a in atoms],float)
    radii=np.array([vdw_radius(a.element, custom) for a in atoms],float)
    meta=[(a.chain,a.resname,a.resi) for a in atoms]

    occ=(occupancy or "hydro").lower()
    if occ.startswith("hydro"): occ="hydro"
    elif occ.startswith("elect"): occ="electro"
    else: occ="hydro"

    if centerline=='curved':
        centers,u,L=construct_centers_curved(coords,radii,c_bot,c_top,step,curve_radius,curve_iters)
        rows=profile_along_centers(coords,radii,centers,eps,meta,hydroscale,electroscale,occ)
    else:
        rows,u,L=profile_along_axis(coords,radii,c_bot,c_top,step,eps,meta,adaptive,slope_thresh,max_refine,hydroscale,electroscale,occ)

    outp=Path(outprefix)
    if outp.suffix.lower()=='.csv': outp=outp.with_suffix('')
    csv_path     = outp.with_suffix('.csv')
    mesh_path    = outp.with_name(outp.stem + '_mesh.pdb')
    center_path  = outp.with_name(outp.stem + '_centerline.pdb')
    summary_path = outp.with_name(outp.stem + '_summary.json')

    # volumes & conductance proxy (same as CLI v8)
    volume_geom=_trap_volume_A3(rows,0.0); volume_access=_trap_volume_A3(rows,float(probe))
    G_S, R_pore, R_access = _conductance_proxy_S(rows, conductivity)
    G_nS = G_S * 1e9

    # passability (optional file like CLI)
    pass_radii={}
    if passable_json:
        try:
            with open(passable_json,'r') as pf: pass_radii=json.load(pf)
        except Exception: pass
    if not pass_radii: pass_radii={'water':1.4,'na':1.02,'k':1.38,'ca':1.00}
    pass_report={}
    for sp,radA in pass_radii.items():
    ...
    write_mesh_pdb(mesh_path, rows, axis_u=(c_top - c_bot), rings=rings)
    write_centerline_pdb(center_path, rows)
    with open(summary_path,'w') as jf:
        json.dump({'pdb':str(pdb_path.resolve()),'top':top,'bottom':bottom,'centerline':centerline,'step_A':float(step),'eps_A':float(eps),
                   'probe_A':float(probe),'length_A':float(rows[-1]['s_A'] if rows else 0.0),
                   'volume_geometric_A3':float(volume_geom),'volume_accessible_A3':float(volume_access),
                   'conductivity_S_per_m':float(conductivity),
                   'R_pore_ohm':float(R_pore),'R_access_ohm':float(R_access),'R_total_ohm':float((R_pore+R_access)),
                   'G_nS':float(G_nS),'passability':pass_report,
                   'min_radius_A':float(min([r['radius_A'] for r in rows]) if rows else 0.0),
                   'num_samples':len(rows),'occupancy_metric':occ,'hydroscale':hydroscale,'electroscale':electroscale}, jf, indent=2)

    return {
        "csv": str(csv_path),
        "mesh_pdb": str(mesh_path),
        "centerline_pdb": str(center_path),
        "summary_json": str(summary_path),
        "g_proxy_S": float(G_S),
        "R_proxy_Ohm": float(R_pore + R_access)
    }
